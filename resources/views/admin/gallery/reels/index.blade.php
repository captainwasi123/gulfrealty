            ...
            <div class="card-header">
              <div class="row">
                <div class="col-md-9 searchbar">
                  <input type="text" name="retailer" placeholder="Search for Reels..." class="form-control searchBlog">
                  <i class="fas fa-search"></i>
                </div>
                <div class="col-md-3">
                  <a href="javascript:void(0)" class="btn btn-primary pull-right addblogButton" title="Add Blog"><i class="fas fa-plus"></i> Add Reel</a>
                </div>
              </div>
            </div>
          </div>
...
<div class="modal fade" id="addBlogFormModal">
  <div class="modal-dialog">
    <div class="modal-content">
      <form id="add_blog_form" action="{{route('admin.gallery.reels.create')}}">
        @csrf
        <div class="modal-header">
          <h4 class="modal-title">Add Reel</h4>
          ...
          <div class="row">
            <div class="col-md-12">
              <div class="form-group">
                ...
                <input type="text" class="form-control" name="reel_link" required>
              </div>
            </div>
          </div>
        </div>
...
<script>
  ...
  $(function() {
    ...
    $(document).on('keyup', '.searchBlog', function() {
      ...
      var url = "{{URL::to('/admin/panel/gallery/reels/search')}}/" + val;
    ...
    $(document).on('click', '.deleteEpisode', function() {
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          $.get("{{URL::to('/admin/panel/gallery/reels/delete')}}/" + id, function(data) {
            Toast.fire({
              icon: 'success',
    ...
    $(document).on('click', '.editBlog', function() {
      ...
      $.get("{{URL::to('/admin/panel/gallery/reels/edit')}}/" + id, function(data) {
  ...
  function loadBlogs() {
    ...
    $pu = ($pu == 0 ? '' : '?page='.$pu);
    @endphp
    var url = "{{route('admin.gallery.reels.load').$pu}}";
